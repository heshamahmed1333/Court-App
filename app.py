...
with st.sidebar:
    ...
    if st.button("إضافة الطعن"):
        st.session_state.cases.append({'رقم الطعن': c_no, 'السنة': c_yr, 'اسم الطاعن': c_ap, 'المحكمة المصدر': c_ct, 'التهمة': c_ch, 'منطوق الحكم': "", 'حضور المحامين': ""})
        st.rerun()
...
with t2:
    ...
    if df_f.empty:
        ...
        with col_a:
            hukm = st.text_area("منطوق الحكم", value=item.get('منطوق الحكم', ""), key=f"h_{item['م']}")
        with col_b:
            hodoor = st.text_area("حضور المحامين", value=item.get('حضور المحامين', ""), key=f_idx:=f"ho_{item['م']}")
        ...
        # --- المعاينة الفورية (الجدول يظهر هنا دائماً) ---
        st.divider()
